...
export class ApprovalWorkflow implements IApprovalWorkflow {
  ...
  async createRequest(request: Omit<ApprovalRequest, 'id' | 'createdAt' | 'expiresAt' | 'status'>): Promise<ApprovalRequest> {
    ...
    const full: ApprovalRequest = {
      ...request, id, createdAt: now, expiresAt: new Date(now.getTime() + 24 * 60 * 60 * 1000), status: 'pendente',
    };
  ...
  getByEmail(emailId: string): ApprovalRequest[] {
    return Array.from(this.requests.values()).filter(r => r.emailId === emailId);
  }
