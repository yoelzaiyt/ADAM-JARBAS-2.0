...
// ============================================================
// IMAGE GENERATION - via OpenRouter unified chat completions
// ============================================================
const IMAGE_MODELS = ['google/gemini-2.5-flash-image', 'google/gemini-3.1-flash-lite-image'];

async function generateImage(prompt, model, referenceImages) {
  const config = PROVIDERS.openrouter;
  if (!config.apiKey) throw new Error('No API key for provider: openrouter');

  const selectedModel = IMAGE_MODELS.includes(model) ? model : IMAGE_MODELS[0];
  const startTime = Date.now();
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (path === '/api/v1/images/generate' && req.method === 'POST') {
      ...
      const result = await generateImage(prompt, model, referenceImages);
      sendJSON(res, 200, result);
      return;
    }
